...
class lcd_weact:
    ...
    def show_text(
        ...
        if size == 12:
            font = ImageFont.truetype(Path(__file__).parent / "fusion-pixel-12px-monospaced-zh_hans.otf", size)
        elif size == 10:
            font = ImageFont.truetype(Path(__file__).parent / "fusion-pixel-10px-monospaced-zh_hans.otf", size)
        elif size == 8:
            font = ImageFont.truetype(Path(__file__).parent / "fusion-pixel-8px-monospaced-zh_hans.otf", size)
        else:
            font = ImageFont.truetype(Path(__file__).parent / "SourceHanSansCN-Normal.otf", size)
        ...
        return self.show_bitmap(xs, ys, image,True)
...
class tk_gui:
    def __init__(self, lcd: lcd_weact) -> None:
        ...
        if self.lcd.auto_open() == False:
            device_state = "Device not connected !"
            self.device_connected = False
        else:
            device_state = "Device connected !"
            self.device_connected = True

        self.window = tkinter.Tk()
        self.window.title("WeAct Studio Display Configuration")
        ...
        device_refresh_button = ttk.Button(
            self.main_frame, text="Refresh", command=self.on_refresh_device_open
        )
        device_refresh_button.grid(row=0, column=2, padx=5, pady=5, sticky="W" + "E")

        orient_label = ttk.Label(self.main_frame, text="Orientation")
        ...
        self.brightness_label = ttk.Label(self.main_frame, text="Brightness")
        ...
        u_orient_label = ttk.Label(self.main_frame, text="Unconnect Orientation")
        ...
        self.u_brightness_label = ttk.Label(self.main_frame, text="Unconnect Brightness")
        ...
        self.device_reset_button = ttk.Button(
            self.main_frame, text="Device Reset", command=self.on_device_reset
        )
        ...
        self.Version_string.set("Version: ")
        ...
        self.Serial_Num_string.set("Serial Num: ")
    ...
    def refresh_device_state(self):
        if self.device_connected == True:
            try:
                ...
                if value != None:
                    self.Version_string.set("Version: " + value.decode())

                value = self.lcd.get_device_serial_num()
                if value != None:
                    self.Serial_Num_string.set("Serial Num: " + value.decode())
    ...
    def window_refresh(self):
        if self.device_connected == True:
            try:
                ...
                traceback.print_exc()
                self.lcd.close()
                device_state = "Device Unconnected !"
                self.device_connected = False
                self.device_state_string.set(device_state)
